...
public class MyPicture extends SimplePicture
{
  ...
  public void posterize(int levels)
  {
    ...
    Pixel[] pixels = this.getPixels();
    double bucket = 256.0 / levels;

    for (Pixel p : pixels) {
      int r = (int)(Math.floor(p.getRed() / bucket) * bucket + bucket / 2);
      int g = (int)(Math.floor(p.getGreen() / bucket) * bucket + bucket / 2);
      int b = (int)(Math.floor(p.getBlue() / bucket) * bucket + bucket / 2);
      p.setColor(new Color(clamp(r), clamp(g), clamp(b)));
    }
  }
